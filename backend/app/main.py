...
app = FastAPI(title="Smart Queue AI API", version="1.0.0")
...
def startup():
    ...
    try:
        if db.query(Service).count() == 0:
            db.add_all([
                Service(name="Account Opening", base_duration=18),
                Service(name="Cash Deposit", base_duration=7),
                Service(name="Cash Withdrawal", base_duration=8),
                Service(name="Loan Enquiry", base_duration=20),
                Service(name="Document Verification", base_duration=12),
            ])
...
async def complete_token(token_id: int, payload: CompleteRequest, db: Session = Depends(get_db)):
    ...
    next_token = (
        db.query(Token)
        .filter(Token.status == "waiting", Token.counter_id == token.counter_id)
        .order_by(Token.created_at.asc())
        .first()
        if token.counter_id else None
    )
    if next_token:
        counter.current_token_id = next_token.id if counter else None
